    ...
    if ($action === 'submit_assignment') {
        ...
        if (isset($_FILES['submission_file']) && $_FILES['submission_file']['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES["submission_file"]["tmp_name"], $target_file)) {
                ...
                award_achievement($conn, $user_id, 2);
            }
        }
...
$assignments = $conn->query("SELECT a.*, c.Class_Name, COUNT(asd.Submission_ID) as submission_count FROM Assignments a LEFT JOIN Classes c ON a.Class_ID = c.Class_ID LEFT JOIN Assignment_Submission_Data asd ON a.Assignment_ID = asd.Assignment_ID GROUP BY a.Assignment_ID ORDER BY a.Assignment_DueDate DESC")->fetch_all(MYSQLI_ASSOC);
$user_submissions = [];
